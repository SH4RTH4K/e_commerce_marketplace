...
| **Admin System Health redirects or returns 403** | Sign in at `/admin/login` with the account whose database `role` is `admin`. The application accepts legacy `Admin`, `ADMIN`, and whitespace-padded role values; upload the latest source and clear the config/cache. |
...
### Admin System Health access on cPanel

The System Health and Git Repository tools are restricted to the super-admin account.
After uploading the latest release, run the following from the Laravel project root using
the cPanel PHP 8.3 binary:
...
If the account can log in but still receives a 403, inspect the `users.role` value in the
database. It should identify the intended account as `admin`; do not grant this access to
customer accounts. A user who is already logged in should log out and back in after the
deployment so the new role check is used.
