...
export default function SupplierProducts({ products = [], suppliers = [], categories = [], stock_filter = 'all', stock_operator = '', stock_value = '', category_filter = '', import_status_filter = '', pagination = {} }) {
  ...
  const [importStatus, setImportStatus] = useState(import_status_filter);
  ...
  const clearFilters = () => { setOperator(''); setValue(''); setCategory(''); setImportStatus(''); setSelected([]); router.get('/admin/dropshipping/products', {}, { preserveState: true, preserveScroll: true }); };
  ...
  return <DropshippingSubpage title="Supplier Products" description="Browse the local supplier catalogue mirror and import selected products as drafts.">
    ...
    <form onSubmit={applyFilters} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5"><div className="flex flex-col gap-3 lg:flex-row lg:items-end"><label className="flex-1 text-xs font-semibold text-gray-600">Category<select value={category} onChange={event => setCategory(event.target.value)} className="mt-1 w-full border border-gray-200 rounded-lg px-2.5 py-2 text-sm bg-white"><option value="">All categories</option>{categories.map(item => <option key={item} value={item}>{item}</option>)}</select></label><label className="text-xs font-semibold text-gray-600">Import status<select value={importStatus} onChange={event => setImportStatus(event.target.value)} className="mt-1 w-full border border-gray-200 rounded-lg px-2.5 py-2 text-sm bg-white"><option value="">All</option><option value="imported">Already imported</option><option value="not_imported">Import draft</option></select></label><label className="text-xs font-semibold text-gray-600">Stock comparison<select value={operator} onChange={event => setOperator(event.target.value)} className="mt-1 w-full border border-gray-200 rounded-lg px-2.5 py-2 text-sm bg-white"><option value="">Any stock</option><option value="gt">Greater than</option><option value="lt">Less than</option><option value="eq">Equal to</option></select></label><label className="text-xs font-semibold text-gray-600">Stock value<input type="number" min="0" step="any" value={value} onChange={event => setValue(event.target.value)} placeholder="e.g. 10" className="mt-1 w-full border border-gray-200 rounded-lg px-2.5 py-2 text-sm" /></label><button type="submit" className="px-4 py-2 rounded-lg bg-orange-500 hover:bg-orange-600 text-sm font-semibold text-white">Apply filters</button><button type="button" onClick={clearFilters} className="px-4 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 text-sm font-semibold text-gray-700">Clear</button></div></form>
    {products.length === 0 && <section className="bg-blue-50 border border-blue-200 rounded-2xl p-5"><h2 className="font-bold text-blue-900">No supplier products match these filters</h2><p className="text-sm text-blue-800 mt-1">Change the category, import status, or stock comparison and try again.</p></section>}
