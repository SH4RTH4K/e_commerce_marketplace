  ...
  <div style="max-width:560px;margin:0 auto;padding:32px 16px;">
    ...
    <div style="background:#fff;padding:28px 24px;border-radius:0 0 12px 12px;border:1px solid #e5e7eb;border-top:0;">
      ...
      <div style="background:#fff4ec;border:1px solid #fed7aa;border-radius:10px;padding:14px 20px;margin-bottom:20px;">
        <p style="margin:0;font-size:13px;color:#92400e;text-transform:uppercase;letter-spacing:0.05em;">Order Number</p>
        <p style="margin:4px 0 0;font-size:22px;font-weight:800;color:#f15a24;letter-spacing:0.04em;">{{ $order->order_number }}</p>
      </div>
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse;margin-bottom:20px;">
        <thead>
          <tr style="background:#f9fafb;">
            ...
            <th style="padding:8px 10px;text-align:right;font-size:12px;color:#6b7280;font-weight:600;border-bottom:1px solid #e5e7eb;">Price</th>
          </tr>
        </thead>
        <tbody>
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:24px;">
        <tr>
          ...
          <td style="padding:4px 0;text-align:right;font-size:13px;">{{ money($order->subtotal) }}</td>
        </tr>
        @if ((float)$order->discount_amount > 0)
        <tr>
          <td style="padding:4px 0;font-size:13px;color:#6b7280;">Discount</td>
          <td style="padding:4px 0;text-align:right;font-size:13px;color:#16a34a;">&minus; {{ money($order->discount_amount) }}</td>
        </tr>
        @endif
        <tr>
          <td style="padding:4px 0;font-size:13px;color:#6b7280;">Shipping</td>
          <td style="padding:4px 0;text-align:right;font-size:13px;">{{ money($order->shipping_charge) }}</td>
        </tr>
        @if ((float)$order->tax > 0)
        <tr>
          <td style="padding:4px 0;font-size:13px;color:#6b7280;">Tax</td>
          <td style="padding:4px 0;text-align:right;font-size:13px;">{{ money($order->tax) }}</td>
        </tr>
        @endif
      ...
      <div style="background:#f9fafb;border-radius:8px;padding:14px 16px;margin-bottom:20px;">
        <p style="margin:0 0 6px;font-size:12px;color:#6b7280;text-transform:uppercase;letter-spacing:0.05em;">Delivery Address</p>
        <p style="margin:0;font-size:14px;">{{ $order->shipping_address }}, {{ $order->city }}</p>
        <p style="margin:4px 0 0;font-size:13px;color:#6b7280;">Payment: <strong>{{ $order->paymentMethodLabel() }}</strong></p>
      </div>

      <p style="margin:0;font-size:13px;color:#94a3b8;">
        We will notify you when your order is on its way. If you have any questions, feel free to contact us.
      </p>
    </div>
